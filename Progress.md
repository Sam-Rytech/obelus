...
| Sep 24 | **Functions run in Frankfurt (`fra1`)** | Measured from production: in Vercel's US region Binance answers 451 and Bybit 403, so neither could ever be checked — and a 451 read as "not listed" produced "Not on Binance yet" for a coin trading there. From fra1 all 16 sources answer; Redis costs ~250 ms instead of ~12 ms |
| Sep 24 | Region blocks (451/403), 401 and 429 are source errors, never "no market"; evidence quotes the HTTP status | Only an exchange's own 400/404 or an empty result is an answer. Health no longer counts a block as "up" |
| Sep 24 | Versioned cache keys when a parser changes (`certik:v2`, `binance:sym:v2`) | A 24 h cache kept serving the pre-fix CertiK excerpt with leaked HTML |
| Sep 24 | **Groq added; extraction is a cross-provider chain** (Sam supplied a Groq key) | Gemini's free tier was down for hours. On the test announcement: qwen3.8-27b 8/8 exact quotes, 4.6 s; gpt-oss-120b 7/8 (paraphrased "listed on WEEX and BingX" — guard dropped it), 2.9 s; gpt-oss-20b malformed JSON. Chain: qwen → gpt-oss-120b → Gemini ×2, 45 s shared deadline. Every failure falls through — including 400/401 (providers differ) and output that fails schema validation. Full local check: 12.0 s |
| Sep 24 | Gemini: Flash-Lite primary, thinking `low`, pipeline deadline | Production timed out. Default thinking cost 2,655 tokens (15.9 s → 3.6 s at `low`). A temporary in-production diagnostic then showed the free tier's latency depends on where the call comes from: from Vercel iad1, Flash-Lite ~18 s steady, 3.6-flash 26.6 s for "OK" then 503. The check phase now shrinks to finish inside Vercel's 60 s |
...
**Also Sep 24:** the site was rebuilt around the annotated report (decision log), and verified at desktop and
phone width, in light and dark, with live checks through the new UI on production.
